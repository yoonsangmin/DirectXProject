	...
	float terrainWidth, float terrainHeight)
{
	...
	if (!m_MiniMapBitmap->Initialize(device, screenWidth, screenHeight, (WCHAR*)L"../directx project/data/minimap/minimap.dds", 154, 154))
	{
		return false;
	}
	...
	// 포인트 비트 맵 객체를 초기화합니다.
	if (!m_PointBitmap->Initialize(device, screenWidth, screenHeight, (WCHAR*)L"../directx project/data/minimap/point.dds", 3, 3))
	{
		return false;
	}
